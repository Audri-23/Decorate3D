...
export function SellerEscrowPanel({ user }) {
  ...
  async function loadOrders(showLoading = true) {
    ...
    try {
      // Fetch all orders belonging to this seller
      const sellerEmailQuery = user?.email ? `sellerEmail=${encodeURIComponent(user.email)}` : '';
      const response = await fetch(`/api/escrow/orders?${sellerEmailQuery}`);
        ...
        return (
          <div key={order._id} className="bg-white rounded-2xl border border-[#E5DEC9] shadow-sm overflow-hidden">
            ...
            <div className="p-5 space-y-1">
              ...
              <p className="text-xs text-gray-400 font-mono truncate">Buyer: {order.buyerEmail}</p>
              <p className="text-xs text-gray-400 font-mono truncate">Stripe ID: {order.stripePaymentIntentId}</p>
            </div>

            {/* Money Breakdown Row — stacked on mobile */}
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="text-sm font-bold text-emerald-700">
                        {order.escrowStatus === 'SPLIT_RESOLVED' ? 'Dispute Settled & Escrow Paid!' : 'Escrow Unlocked Successfully!'}
                      </p>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    <input
                      ...
                      className="w-full sm:w-32 px-4 py-3 text-center font-mono text-xl font-bold border border-[#E5DEC9] rounded-xl focus:outline-none focus:border-[#A17A16] bg-gray-50 tracking-widest min-h-[44px]"
                    />
                    <button
                      onClick={() => handleVerifyOtp(order._id)}
                      className="gold-gradient-btn px-5 py-3 rounded-xl font-bold text-sm flex items-center justify-center space-x-2 shadow-md min-h-[44px]"
                    >
